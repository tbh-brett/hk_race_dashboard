...
def page_overview():
    ...
    # ── Section 2: Model's most confident picks ────────────
    st.markdown("### 🎯 Model's Confident Picks")
    st.caption("Races where the model has clearest separation between top pick and field")
    ...
    for race in races:
        ...
        second = picks[1]
        third = picks[2]
        gap_1_2 = (second.get("projected_time", 0) or 0) - (top.get("projected_time", 0) or 0)
        wp_top = top.get("win_prob", 0) or 0
        wp_second = second.get("win_prob", 0) or 0
        wp_gap = wp_top - wp_second or 0
        # Confidence score: combination of time gap and probability gap
        conf_score = gap_1_2 * 10 + wp_gap * 0.5
        flags = top.get("flags", [])
        has_dec = any("DEC" in str(f) for f in flags)

        confident_races.append({
            "race": race["race_number"],
            "dist": race.get("distance", "?"),
            "race_class": race.get("race_class", "?"),
            "pace": race.get("pace", "?"),
            "top_horse": top["horse_name"],
            "top_rank": top.get("rank", 1),
            "top_wp": wp_top,
            "top_time": top.get("projected_time"),
            "top_draw": top.get("draw"),
            "top_jockey": top.get("jockey", ""),
            "top_weight": top.get("weight"),
            "top_flags": flags,
            "second_horse": second["horse_name"],
            "second_wp": wp_second,
            "third_horse": third["horse_name"],
            "third_wp": third.get("win_prob", 0),
            "gap_1_2": gap_1_2,
            "wp_gap": wp_gap,
            "conf_score": conf_score,
            "has_decline": has_dec,
            "n_runners": race.get("runners", len(picks)),
        })

    confident_races.sort(key=lambda x: x["conf_score"], reverse=True)
    # Show top ~5 most confident (or those above threshold)
    shown = 0
    for cr in confident_races:
        if shown >= 5 and cr["conf_score"] < 1.5:
            break
        # Skip races where top pick is flagged declining
        cls = f'C{cr["race_class"]}' if cr["race_class"] and str(cr["race_class"]).isdigit() and int(str(cr["race_class"])) > 0 else "Grp"
        col1, col2 = st.columns([3, 1])
        with col1:
            flag_html = ""
            for fl in cr["top_flags"]:
                css = "flag-dec" if "DEC" in str(fl) else "flag-imp" if "IMP" in str(fl) else "flag-u"
                flag_html += f' <span class="flag-tag {css}">{fl}</span>'

            st.markdown(
                f'**R{cr["race"]}** {cr["dist"]}m {cls} &nbsp;·&nbsp; '
                f'Pace: {cr["pace"]} &nbsp;·&nbsp; {cr["n_runners"]} runners'
                f'<br><span style="font-size:1.15em;font-weight:700">{cr["top_horse"]}</span>'
                f' &nbsp; ({cr["top_jockey"]}) &nbsp; Dr {cr["top_draw"]} &nbsp; {cr["top_weight"]}lb'
                f'{flag_html}',
                unsafe_allow_html=True,
            )
            st.caption(
                f'Win% {cr["top_wp"]:.1f} &nbsp;→&nbsp; '
                f'2nd: {cr["second_horse"]} ({cr["second_wp"]:.1f}%) &nbsp; '
                f'3rd: {cr["third_horse"]} ({cr["third_wp"]:.1f}%) &nbsp; '
                f'Gap: {cr["gap_1_2"]:.2f}s'
            )
        with col2:
            score_colour = "#22c55e" if cr["conf_score"] >= 2 else "#f59e0b" if cr["conf_score"] >= 1 else "#888"
            st.markdown(
                f'<div style="text-align:center;padding:8px">'
                f'<span style="font-size:1.8em;font-weight:700;color:{score_colour}">'
                f'{cr["conf_score"]:.1f}</span>'
                f'<br><span style="font-size:0.7em;opacity:0.5">CONF SCORE</span></div>',
                unsafe_allow_html=True,
            )
            if st.button("View Race →", key=f"ov_goto_r{cr['race']}", type="tertiary"):
                st.session_state["nav_page"] = "Race Day"
                st.session_state["rd_active_race"] = cr["race"]
                st.rerun()
        shown += 1

    if shown == 0:
        st.caption("No races with sufficient data for confident picks.")

    st.markdown("---")

    # ── Section 3: Quick meeting stats ─────────────────────
