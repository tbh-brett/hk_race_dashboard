...
RACE_CARD    = BASE / "racecards" / "racecard_20260422.xlsx"
...
OUT_PDF  = BASE / "reports" / "race_day_report_20260422_v4.4.pdf"
OUT_TEXT = BASE / "reports" / "race_day_analysis_20260422_v4.4.txt"
DB_FILE  = _find_data_file("hkjc_results_updated.xlsx")

MEETING_TITLE = "HAPPY VALLEY — WEDNESDAY, 22 APRIL 2026"
MEETING_VENUE = "HV"
...
def compute_speed_map(df, race, pace_label="Normal"):
    ...
    for h in horses:
        ...
        # Empirical pace × style bonus (seconds) — folded into smap_advantage
        # so it flows into projected_time via SMAP_TIME_COEFF. Negative = faster.
        try:
            from pace_utils import pace_style_bonus_seconds
            pace_bonus_s = pace_style_bonus_seconds(
                pace_label, h.get("dominant_style", "Midfield"),
                distance, venue,
            )
        except Exception:
            pace_bonus_s = 0.0
        ...
        if pace_bonus_s:
            ...
            h["smap_notes"] = (
                f"{h['smap_notes']}; pace×style {pace_bonus_s:+.2f}s"
            )
...
def main():
    ...
    for (df, pace_label, pace_score, pace_reasons, leader_names, smap), race in zip(all_results, races):
        ...
        for b in beneficiaries:
            bh = next(
                (s for s in smap if s["horse_name"] == b["horse_name"]),
                {},
            )
            beneficiary_list.append({
                "horse_name": b["horse_name"],
                "horse_no": b["horse_no"],
                "style": bh.get("dominant_style", ""),
                "advantage": bh.get("smap_advantage", 0.0),
                "pace_style_bonus_s": bh.get("pace_style_bonus_s", 0.0),
                "reason": b.get("reason", "") or "; ".join(b.get("reasons", [])),
            })
